...
params <- expand.grid(
	...
	enriched_paths_only = c(TRUE, FALSE), 
	pathway = c("kegg", "go"),
	sim = c(1:1),
	gene_overlap = NA, 
	pathway_overlap = NA, 
...
perform_enrichment <- function(identified_genes, pathway_df, background_genes) 
{
    ...
    len_up <- length(unique_pathways)
    enriched_paths <- map_dfr(seq_along(unique_pathways), function(x) {
        message("Pathway ", x, " of ", len_up)
...
sim_func <- function(n_genes, ewas_power, gwas_power, percent_ewas_causal, database, enriched_paths_only) 
{
	# simulation function

	pathway_dat <- get(paste0(database, "_terms"))
	# sample genes
	causal_trait_genes <- sample(all_genes$ensembl_gene_id, n_genes)
	gwas_genes <- sample(trait_genes, causal_trait_genes * gwas_power)
	...
	ewas_genes <- c(sample(causal_trait_genes, n_ewas_causal_genes), 
					sample(all_genes$ensembl_gene_id, n_ewas_genes - n_ewas_causal_genes))
	# perform gene overlap test
	outg <- overlap_test(ewas_genes, gwas_genes, all_genes$ensembl_gene_id)
	# perform enrichment analysis if needed 
	if (enriched_paths_only) {
		gwas_enrich <- perform_enrichment(gwas_genes, pathway_dat, all_genes$ensembl_gene_id)
		gwasp <- gwas_enrich %>%
			dplyr::filter(adj_enrich_p < 0.05) %>%
			pull("pathway_id")
		ewas_enrich <- perform_enrichment(ewas_genes, pathway_dat, all_genes$ensembl_gene_id)
		ewasp <- ewas_enrich %>%
			dplyr::filter(adj_enrich_p < 0.05) %>%
			pull("pathway_id")
	} else {
	...
	# perform overlap tests for pathways
	outp <- overlap_test(ewasp, gwasp, pathwas_dat$pathway_id)
	# write out results
	out <- list(gene_res = outg, 
				gene_overlap = sum(ewas_genes %in% gwas_genes),
...
sim_func2 <- function(n_genes, ewas_power, gwas_power, percent_ewas_causal) {
	# simulation function
...
# * power to detect correlation/overlap between the GWAS (or EWAS, doesn't matter) of the same trait given differing powers of the study
# 	+ Take range for N genes from range of actual EWAS/GWAS (roughly from 50 to 2500)
# 	+ Essentially simulation will be: Here are X genes. Study one detected X genes and study two detected Y genes. Vary X and Y and compare power each of the methods have to detect overlap/correlation (will need to create null for methods 1 and 2)

# So these simulations should show which methods we'd see overlap when we expect it (i.e. when X is very similar to Y) and when we wouldn't expect it (i.e. when there is no similarity between X and Y) as well as scenarios in between. 
