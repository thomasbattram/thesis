...
_Interpretation_ -- ignoring for this section. Can do high level in discussion

_Rationale_

Next we estimated (see XXX) the variance captured by each association to gauge whether DNA methylation and complex traits covary substantially.
...
It has previously been suggested that sites at which DNA methylation variability is low should be removed (REF). This is because, if total variation is low then the technical variation to biological variation ratio will be greater and thus any association with a complex trait is more likely to be due to technical artefacts. However, selection would dictate that phenotypes (including DNA methylation) that have a large effect would be selected for (if they had a positive impact on fitness) or against (if they had a negative impact on fitness). Therefore, the largest effects may be missed by removing CpG sites with low variances.

_Results_

There was strong evidence of an inverse association between variance at a CpG site and effect size (P = XXX, __Table \@ref(tab:cpg-chars-tab)__),

_Interpretation_

suggesting that removal of these sites might lead to loss of important biological information.
...
There was strong evidence of an association between mean DNA methylation levels and negative effect sizes (P = XXX, __Table X__) and an inverse association between mean methylation levels and positive effect sizes (P = XXX, __Table \@ref(tab:cpg-chars-tab)__).

_Interpretation_

This suggests that changes away from the extreme ends of the distribution are likely to have larger effects.

_Rationale_

DNA methylation changes are heritable (REF), and DNA methylation could mediate the effects of genotype on complex traits or genotype might confound the association between DNA methylation on complex traits.  

_Results_

There was evidence that effect sizes tended to be greater in more heritable sites (P = XXX, __Table X__). 

_Variance explained results_

The variance in effect size estimates explained by DNA methylation variability, mean methylation level, and heritability, was XXX (__Table \@ref(tab:cpg-chars-tab)__).  
...
Identifying modifiable molecular traits that mediate the effect of complex traits on disease is something that motivates a lot of molecular epidemiology research (REFs). Having a database of associations between DNA methylation and various traits and diseases may enable easy identification of potential mediators that warrant follow-up. Overall, DNA methylation at XXX CpGs are associated with multiple traits. If we just look at the CpG that was identified in the most EWAS, XXX, we can see that methylation at that site associated with weight-related traits such as body mass index and waist-hip ratio, XXX metabolites and type 2 diabetes. CHECK MOST RECENT EWAS TO SEE IF THEY TEST ANY MEDIATING STUFF. Either "this warrants follow-up" OR "this highlights the potential to use these databases for discovering important pathways between traits."

### Biased results {#biased-results}

The potential biases in EWAS have been well documented (REFs) and were discussed at length in __Section X__. It is encouraging that the majority of studies include batch effects cell composition in at least one of their models (X%) and further, the majority of studies that didn't are older (X% before 2018). 

Replication rates also varied largely from EWAS to EWAS...
...
Characteristics of DNA methylation discovered in experimental studies, such as its association with gene expression, were used to select sites to measure DNA methylation (REF). Further, studies have suggested selecting from those sites commonly measured, CpGs that have certain characteristics such as high variance (REFs). By exploring EWAS results one might obtain further understanding in how future technologies and studies that utilise these technologies could be designed.

* Enrichment stuff here

In this study, it was found there is strong evidence that DNA methylation variance inversely correlates with effect size, suggesting that excluding sites with lower variances may prevent discovery of DNA methylation changes strongly associated with a trait. H

There was strong evidence that the mean methylation level, variance of methylation and heriatbility at each site was associated with the effect size from EWAS. Further, sites associated with traits at P < 1x10^-7^ were also enriched in X region of the genome. 
